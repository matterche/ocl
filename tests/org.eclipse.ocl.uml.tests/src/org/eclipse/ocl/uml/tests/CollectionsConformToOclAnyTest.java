 ...
 * </copyright>
 *
 * $Id: CollectionsConformToOclAnyTest.java,v 1.1.2.1 2010/01/30 22:27:09 ewillink Exp $
 */
...
public class CollectionsConformToOclAnyTest
	...
	public void test_collections_conform_to_OclAny_290680() {
		...
		try {
			OCLExpression<Classifier> expr = helper.createQuery("Sequence{OrderedSet{'a', 'b'}}->oclIsKindOf(OclAny)");
...
//			assertEquals(2, ocl.evaluate("", expr));
			...
			assertEquals(Arrays.asList(1, 2), ocl.evaluate("", expr));
		} catch (Exception e) {
	...
	public void test_mixed_element_types_in_collections_290680() {
		...
		try {
			OCLExpression<Classifier> expr = helper.createQuery("Sequence{1, OrderedSet{'a', 'b'}, 'c'}->oclIsKindOf(OclAny)");
			assertEquals(true, ocl.evaluate("", expr));
	...
	public void test_collections_oclIsInvalid_290680() {
		...
		try {
			OCLExpression<Classifier> expr = helper.createQuery("Sequence{1, 2}->oclIsInvalid()");
			assertEquals(false, ocl.evaluate("", expr));
	...
	public void test_collections_oclIsUndefined_290680() {
		...
		try {
			OCLExpression<Classifier> expr = helper.createQuery("Sequence{1, 2}->oclIsUndefined()");
			assertEquals(false, ocl.evaluate("", expr));
	...
	public void test_collections_oclAsType_290680() {
		...
		try {
			OCLExpression<Classifier> expr = helper.createQuery("Sequence{1, 2}->oclAsType(OclAny).oclIsUndefined()");
			assertEquals(false, ocl.evaluate("", expr));
	...
	public void test_def_collection_operation_on_OclAny_290680() {
		...
		try {
			...
			OCLExpression<Classifier> expr = helper.createQuery("Sequence{1, 2, 3}->size()");
			assertEquals(3, ocl.evaluate("", expr));

			expr = helper.createQuery("Sequence{1, 2, 3}.size()");
			assertEquals(Arrays.asList(1, 1, 1), ocl.evaluate("", expr));
	...
	public void test_def_OclAny_operation_290680() {
		...
		try {
			...
			OCLExpression<Classifier> expr = helper.createQuery("10.foo()");
			assertEquals(1, ocl.evaluate("", expr));
