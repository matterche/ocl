...
public class EvaluationStringOperationTest
	...
	public EvaluationStringOperationTest(String name) {
		super(name);
	}
	...
	public void testStringToLower() {
		if (!"Â´".equals("\u00B4")) { //$NON-NLS-1$ //$NON-NLS-2$
		...
		assertResult(
			"á½€Î´Ï…ÏƒÏƒÎµÏ�Ï‚", "'á½ˆÎ”Î¥Î£Î£Î•ÎŽÎ£'.toLower()");
	}
	...
	public void testStringToUpper() {
		...
		// Ensures word-final sigma and regular sigmas are converted as needed
		assertResult("á½ˆÎ”Î¥Î£Î£Î•ÎŽÎ£", "'á½€Î´Ï…ÏƒÏƒÎµÏ�Ï‚'.toUpper()");
		
		// Sharp s should be mapped to a double S upper case
		assertResult("SS", "'ÃŸ'.toUpper()");
	}
