...
public class DocumentationExamples extends PivotTestSuite
{
	...
	protected void doTestOCLinEcoreTutorialUsingPivot(ResourceSet resourceSet, URI testModelURI) throws Exception {
		...
		try {
			PivotEnvironmentFactory environmentFactory = new PivotEnvironmentFactory(resourceSet.getPackageRegistry(), typeManager);
			OCL ocl = OCL.newInstance(environmentFactory);
			OCLHelper helper = ocl.createOCLHelper();
	
			Ecore2Pivot ecore2pivot = Ecore2Pivot.getAdapter(b2Book.eClass().eResource(), typeManager);
