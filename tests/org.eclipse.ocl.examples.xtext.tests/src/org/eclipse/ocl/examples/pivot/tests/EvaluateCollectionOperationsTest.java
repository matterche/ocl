...
//        	OCLMessages.OperationNotFound_ERROR_, "intersection(Set(Integer))", "Set(OclVoid)");	
		...
		assertSemanticErrorQuery("let s : Set(Integer) = invalid in Set{4}->intersection(s)",
			OCLMessages.UnresolvedOperationCall_ERROR_, new Object[]{"intersection", "Set<UnlimitedNatural>", "Set<Integer>"});
...
//		assertQueryInvalid(null, "let s : String = null in s->size()");
	...
	public void testCollectionSum() {
		...
		assertQueryEquals(null, getClassifierType(metaModelManager.getRealType()), "Bag{4.0, 4, 5}->sum().oclType()");
		assertQueryEquals(null, getClassifierType(metaModelManager.getIntegerType()), "Bag{4, -4, -5}->sum().oclType()");
		assertQueryEquals(null, getClassifierType(metaModelManager.getUnlimitedNaturalType()), "Bag{4, 4, 5}->sum().oclType()");
