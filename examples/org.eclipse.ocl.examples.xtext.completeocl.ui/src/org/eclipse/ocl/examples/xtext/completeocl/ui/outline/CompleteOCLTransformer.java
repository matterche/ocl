 ...
 * </copyright>
 *
 * $Id: CompleteOCLTransformer.java,v 1.3.6.1 2010/10/01 15:21:59 ewillink Exp $
 */
...
{	
	...
	public List<EObject> getChildren(CompleteOCLDocumentCS csElement) {
		List<EObject> contents = new ArrayList<EObject>();
		addContents(contents, csElement.getOwnedImport());
		addContents(contents, csElement.getOwnedLibrary());
		addContents(contents, csElement.getPackages());
		addContents(contents, csElement.getOwnedAnnotation());
		return contents;
	}

	public List<EObject> getChildren(DefCS csElement) {
		List<EObject> contents = new ArrayList<EObject>();
		addContent(contents, csElement.getOwnedExpression());
		addContent(contents, csElement.getOwnedReason());
		return contents;
	}
